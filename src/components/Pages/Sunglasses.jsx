...
const Sunglasses = () => {
  ...
  // Categories for Sunglasses (Category ID 2)
  const categories = [
    'ALL',
    'MENS',
    'WOMENS',
  ...
  const fetchSunglasses = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const sunglasses = data.data.filter(item => Number(item.category_id) === 2);
        setProducts(sunglasses);
        setFilteredProducts(sunglasses);
      } else {
  ...
  useEffect(() => {
    if (activeCategory === 'ALL') {
      ...
      const subcategoryMap = {
        'MENS': 8,        // Adjust these IDs if your actual DB IDs differ
        'WOMENS': 9,
        'KIDS': 10,
        'POLARIZED': 11,
        'PRESCRIPTION': 12,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-stretch">
            <div className="w-full overflow-hidden rounded-sm bg-neutral-200 aspect-[4/5] sm:aspect-[13/14] md:aspect-[14/15]">
              <img 
                src={GalaryGlass}
                alt="Elegant sunglasses collection" 
            ...
            <div className="w-full overflow-hidden rounded-sm bg-neutral-200 aspect-[4/5] sm:aspect-[13/14] md:aspect-[14/15]">
              <img 
                src={GalaryCouple}
                alt="Couple wearing stylish sunglasses" 
                className="w-full h-full object-cover object-center transition-transform duration-500 hover:scale-[1.01]"
              />
            </div>
          </div>
                  ...
                  <div className="w-full flex flex-col mb-6">
                    ...
                    <div className="flex justify-between items-baseline font-sans text-[11px] sm:text-xs text-neutral-400 mt-1">
                      <span className="italic">
                        {product.description ? product.description.substring(0, 60) + '...' : 'Premium Sunglasses'}
                      </span>
                  ...
                  <div className="w-full overflow-hidden rounded-sm bg-neutral-200 aspect-square sm:aspect-[14/13] md:aspect-[13/14] relative">
                    {product.image_url ? (
                      <img 
                        ...
                        onError={(e) => {
                          e.currentTarget.src = "https://images.unsplash.com/photo-1508296695146-257a814070b4?q=80&w=1000&auto=format&fit=crop";
                        }}
                      />
                    ) : (
