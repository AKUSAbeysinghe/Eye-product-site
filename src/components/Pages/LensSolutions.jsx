...
const LensSolutions = () => {
  ...
  // Categories for Lens Solutions & Eye Care (Category ID 4)
  const categories = [
    'ALL',
    'SOLUTION',
    'CLEANER',
  ...
  const fetchLensSolutions = async () => {
    setIsLoading(true);
    setError("");
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const solutions = data.data.filter(item => Number(item.category_id) === 4);
        setProducts(solutions);
        setFilteredProducts(solutions);
      } else {
                  ...
                  <div className="w-full flex flex-col mb-6">
                    ...
                    <div className="flex justify-between items-baseline font-sans text-[11px] sm:text-xs text-neutral-400 mt-1">
                      <span className="italic">
                        {product.description ? product.description.substring(0, 65) + '...' : 'Eye Care Essential'}
                      </span>
