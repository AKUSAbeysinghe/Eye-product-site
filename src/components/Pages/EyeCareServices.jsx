...
const EyeCareServices = () => {
  ...
  // Categories for Eye Care Services (Category ID 6)
  const categories = [
    'ALL',
    'EXAMINATION',
    'VISION TEST',
  ...
  const fetchEyeCareServices = async () => {
    setIsLoading(true);
    setError("");
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const services = data.data.filter(item => Number(item.category_id) === 6);
  ...
  const handleWhatsAppClick = (product) => {
    const message = `Hi! I'm interested in this service:\n\n` +
                    `*${product.name}*\n` +
                    `Price: Rs. ${formatPrice(product.price)}\n\n` +
                    `Please provide more details & availability.`;
  ...
  return (
    <>
      ...
      <section className="bg-[#f7f5f0] text-neutral-900 px-6 py-12 sm:px-12 md:px-16 md:py-20 font-serif">
        <div className="max-w-7xl mx-auto">
          {isLoading && (
            <div className="text-center py-20 text-xl">Loading services...</div>
          )}
                  ...
                  <div className="w-full flex flex-col mb-6">
                    ...
                    <div className="flex justify-between items-baseline font-sans text-[11px] sm:text-xs text-neutral-400 mt-1">
                      <span className="italic">
                        {product.description ? product.description.substring(0, 70) + '...' : 'Professional Eye Care Service'}
                      </span>
