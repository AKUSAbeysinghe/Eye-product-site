...
const ContactLenses = () => {
  ...
  // Categories for Contact Lenses (Category ID 3)
  const categories = [
    'ALL',
    'DAILY',
    'MONTHLY',
  ...
  const fetchContactLenses = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const lenses = data.data.filter(item => Number(item.category_id) === 3);
        setProducts(lenses);
        setFilteredProducts(lenses);
      } else {
                  ...
                  <div className="w-full flex flex-col mb-6">
                    ...
                    <div className="flex justify-between items-baseline font-sans text-[11px] sm:text-xs text-neutral-400 mt-1">
                      <span className="italic">
                        {product.description ? product.description.substring(0, 65) + '...' : 'Premium Contact Lenses'}
                      </span>
